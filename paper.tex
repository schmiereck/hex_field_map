...
\begin{figure}[b]
  \centering
  \includegraphics[width=\columnwidth]{lattice_geometry_2d.png}
  \caption{%
...
3 & 1.010 & $+0.001$ & Near-zero (timelike)   \\
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{dispersion_relation_2d.png}
  \caption{%
    Dispersion relation $E(k)$ in 2+1D.
    \emph{Left}: $E(|\mathbf{k}|)$ along three directions
    ($0°$, $30°$, $60°$) compared to the relativistic curve
    $E = \sqrt{3k^2 + m^2}$.  All three curves overlap, confirming
    6-fold isotropy (error\,$= 0.0000$ for $|k| \le 0.4$).
    \emph{Right}: 2D heatmap $E(k_x, k_y)$; circular symmetry
    confirms isotropy.
  }
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{spacetime_spread_2d.png}
  \caption{%
    Probability density $|\psi(x,y,t)|^2$ in 2+1D at times
    $t = 5, 10, 15, 20$.
    Dashed circles show the light cone $r = \sqrt{3}\,t$.
    The density is strictly causal: no weight lies outside the cone.
  }
  \label{fig:spread}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{dilation_curve.png}
  \caption{%
    Relativistic time dilation.
    Black solid line: classical prediction $\tau/T = \sqrt{1 - v^2/c^2}$.
    Blue circles: quantum prediction $\tauq/T$ for $\sigma = 8$
    ($\sigma_k/m = 0.627$).
    Red squares: phase-slope measurement (overestimates due to broad
    $k$-spread).
    Green triangles: straight-step distribution $\langle\tau_{\rm acc}\rangle$
    (small, since the physical mode is lightlike).
    Residuals in the lower panel show the deviation of the phase measurement
    from $\tauq$.
  }
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{sigma_convergence.png}
  \caption{%
    $\sigma$-convergence of $\tauq/T$ toward the SR curve.
    Main panel: $\tauq/T$ vs.\ $v/c$ for $\sigma = 8, 20, 30$;
    the black solid line is $\sqrt{1 - v^2/c^2}$.
    Lower panel: residuals $({\tauq - \taucl})/\taucl$ in per cent.
    The deviation halves with each doubling of~$\sigma$, confirming
    convergence to the SR prediction in the narrow-packet limit.
  }
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{wavepacket_analysis_2d.png}
  \caption{%
...
The 5-fold degenerate physical band in 2+1D is intriguing.
Five is not a standard spin degeneracy for a Dirac fermion in 2+1D
(which would be 2).
This may indicate an additional internal degree of freedom---possibly a
lattice-specific ``orientation'' quantum number---or a partial suppression of
doublers that leaves a residual degeneracy.
Understanding this structure could reveal a richer spin content than
anticipated.
...
\bibitem{ord1983}
  G.~N.~Ord,
  ``A classical analogue of quantum phase,''
  \emph{J.\ Phys.\ A: Math.\ Gen.}\ \textbf{16}, 1869 (1983).
